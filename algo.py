...
class PSO(Algo):
        ...
        while current_evl < self.evaluation:
            ...
            for p in Particles:
                for d in range(self.dim):
                    lo1 = random.random()
                    lo2 = random.random()
                    p.personal_v[d] = 0.729844 * p.personal_v[d] + lo1 * 1.496180 * (p.personal_c[d] - p.coordination[d]) + lo2 * 1.496180 * (best_p.personal_c[d] - p.coordination[d])
                ...
                if best_p.best < best_so_far:
                    best_so_far = best_p.best
    ...
    def run_algo(self):
        ...
        for i in range(self.run):
            algo = PSO(dim=self.dim, testing=self.testing)
